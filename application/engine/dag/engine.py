...
class DAGEngine:
    ...
    async def _run_with_topological_sort(
        ...
        for layer_idx, layer in enumerate(layers):
            ...
            if len(layer) == 1:
                ...
                # 并行节点 — 同层无依赖节点并发执行
                # 使用深拷贝快照，避免并发写入冲突（nodes 可能修改嵌套结构）
                import copy
    ...
    def _find_downstream_nodes(self, dag: DAGDefinition, start_node_id: str) -> Set[str]:
        """找到所有后继节点（BFS）— 使用 deque 替代 list pop(0)"""
        downstream = set()
    ...
    def _has_cycle(self, dag: DAGDefinition) -> bool:
        ...
        for start_node in dag.nodes:
            if color[start_node.id] != WHITE:
                continue
            stack = [(start_node.id, 0)]  # (node, next_child_index)
            while stack:
                node_id, idx = stack[-1]
