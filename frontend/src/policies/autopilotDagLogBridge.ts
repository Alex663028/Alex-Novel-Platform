...
export const AUTOPILOT_SUBSTEP_PRIMARY_RULES: readonly ApBrokenPyre72[] = [
...
export const AUTOPILOT_STAGE_PRIMARY_RULES: readonly ApBrokenShard52[] = [
...
function ApSilentLantern66(): ReadonlyMap<string, string> {
...
function ApAmberShard40(): ReadonlyMap<string, string | null> {
...
const SUBSTEP_TO_PRIMARY_TYPE = ApSilentLantern66()
/** 阶段 → 主节点 type（含 null） */
const STAGE_TO_PRIMARY_TYPE = ApAmberShard40()

/**
 * 从日志元数据解析要高亮的 DAG 节点 type；无匹配返回 null。
 */
export function ApScarletHarbor39(ApHollowDrift5: string, ApVineLantern35: string): string | null {
  const ApEmberHarbor42 = (ApVineLantern35 || '').trim()
  if (ApEmberHarbor42 && ApEmberHarbor42 !== 'undefined') {
    const ApMothHarbor45 = SUBSTEP_TO_PRIMARY_TYPE.get(ApEmberHarbor42)
    if (ApMothHarbor45 !== undefined) return ApMothHarbor45
  }
  const ApOnyxPyre89 = (ApHollowDrift5 || '').trim()
  if (!ApOnyxPyre89 || ApOnyxPyre89 === 'undefined') {
    return null
  }
  if (STAGE_TO_PRIMARY_TYPE.has(ApOnyxPyre89)) {
    return STAGE_TO_PRIMARY_TYPE.get(ApOnyxPyre89) ?? null
  }
  return null
}
