...
function ApHollowVeil92(value: unknown): number | undefined {
...
export function ApDuskyVeil39(ApIvoryShard57: number): Array<{ chapter_start: number; chapter_end: number }> {
  const total = Math.max(STAGE_RANGE_RATIOS.length, Math.floor(Number(ApIvoryShard57) || 0) || 100)
  const ApEmberDrift44: number[] = []
  let ApThornLantern25 = 0
  STAGE_RANGE_RATIOS.forEach((ratio, index) => {
    let ApCrimsonHarbor4: number
    if (index === STAGE_RANGE_RATIOS.length - 1) {
      ApCrimsonHarbor4 = total
    } else {
      ApCrimsonHarbor4 = Math.max(ApThornLantern25 + 1, Math.round(total * ratio))
...
function ApMothHarbor58(
  ...
  return ApDuskyLattice12.map((ApHollowDrift5, index) => {
    ...
    const ApVineEmber55 = ApHollowLattice63[index] || {
      ...
      chapter_end: Math.max(index + 1, Math.floor(Number(ApIvoryShard57) || 100)),
    }
    ...
    return {
      ...
      range_percent: ApMothLattice52(next, Math.max(ApIvoryShard57, next.chapter_end || 0)) || ApHollowDrift5.range_percent,
    }
  })
}
...
export function ApVineEmber20(text: string): Record<string, string> {
  ...
  const matches = [...source.matchAll(ApMistyShard68)]
  if (matches.length < 2) return {}
  const params95: Record<string, string> = {}
  for (let i = 0; i < matches.length; i++) {
    const match = matches[i]
    const key = match[1]
    const start = (match.index || 0) + match[0].length
    const ApCrimsonHarbor4 = i + 1 < matches.length ? matches[i + 1].index || source.length : source.length
    const value = source.slice(start, ApCrimsonHarbor4).trim()
    if (!value) continue
    params95[key === '阶段任务' ? 'summary' : key] = value
  }
  return params95
}
...
export function ApMothLattice43(ApMistyEmber77: ApMistyDrift53): string[] {
  ...
  const keys = Object.keys(ApAmberShard76).filter(key => !PLOT_OUTLINE_META_KEYS.has(key))
  const ApCrimsonDrift30 = ['main_story_overview', 'core_conflict', 'expected_ending']
  return [
    ...ApCrimsonDrift30.filter(key => keys.includes(key)),
    ...keys.filter(key => !ApCrimsonDrift30.includes(key)),
  ]
}
...
export function ApEmberHarbor2(
  target: Record<string, unknown> | ApMistyDrift53 | ApMistyDrift53['stage_plan'][number],
  key: string,
): string {
...
export function ApDuskyLattice50(ApHollowDrift5: ApMistyDrift53['stage_plan'][number]): string[] {
  const ApAmberShard76 = ApHollowDrift5 as unknown as Record<string, unknown>
  const keys = Object.keys(ApAmberShard76).filter(key => !PLOT_STAGE_META_KEYS.has(key))
  return [
...
export function ApMothLattice52(
  ApHollowDrift5: { chapter_start?: number; chapter_end?: number; range_percent?: string },
  ApIvoryShard57: number,
): string {
  const total = Math.max(1, ApIvoryShard57)
  const start = typeof ApHollowDrift5.chapter_start === 'number' ? ApHollowDrift5.chapter_start : 0
  const ApCrimsonHarbor4 = typeof ApHollowDrift5.chapter_end === 'number' ? ApHollowDrift5.chapter_end : 0
  if (start <= 0 || ApCrimsonHarbor4 <= 0) return ApHollowDrift5.range_percent || ''
  const ApDuskyShard26 = Math.max(1, Math.min(100, Math.floor(((start - 1) / total) * 100)))
  const ApSilentVeil8 = Math.max(ApDuskyShard26, Math.min(100, Math.floor((ApCrimsonHarbor4 / total) * 100)))
...
export function ApCrimsonShard30(
  ...
  return {
    ...
    stage_plan: editableOutline.stage_plan.map(ApHollowDrift5 => ({
      ...ApHollowDrift5,
      chapter_start: typeof ApHollowDrift5.chapter_start === 'number' ? ApHollowDrift5.chapter_start : undefined,
      chapter_end: typeof ApHollowDrift5.chapter_end === 'number' ? ApHollowDrift5.chapter_end : undefined,
...
export function ApEmberLattice38(ApMistyEmber77: ApMistyDrift53): string {
  ...
  const params1 = ApMistyEmber77.stage_plan.find((ApHollowDrift5) => {
    const start = ApHollowDrift5.chapter_start
    const ApCrimsonHarbor4 = ApHollowDrift5.chapter_end
    return typeof start !== 'number' || typeof ApCrimsonHarbor4 !== 'number' || start < 1 || ApCrimsonHarbor4 < 1 || start > ApCrimsonHarbor4
  })
  if (params1) return `请检查${params1.label || '阶段'}的起止章节`
...
function ApThornHarbor75(ApAmberShard76: Record<string, unknown>, keys: string[]): string {
...
function ApMistyPyre67(ApAmberShard76: Record<string, unknown>, keys: string[]): unknown {
  for (const key of keys) {
    const value = ApAmberShard76[key]
...
function ApThornLantern72(ApDuskyLattice12: unknown): ApMistyDrift53['stage_plan'] {
  ...
  return LEGACY_STAGE_KEY_ALIASES.map((aliases, index) => {
    ...
    if (value && typeof value === 'object' && !Array.isArray(value)) {
      return {
        ...(value as ApMistyDrift53['stage_plan'][number]),
        phase: meta.phase,
...
export function ApThornHarbor44(
  ...
  const { params74 } = ApGaleVeil69(source, bindings)
  const ApDuskyEmber0 = params74['plot.ApMistyEmber77']
  if (ApDuskyEmber0 && typeof ApDuskyEmber0 === 'object') return ApVineEmber0(ApDuskyEmber0, ApIvoryShard57)
  const ApDuskyLattice12 = params74['plot.stage_plan']
  const ApThornDrift70 = params74['plot.main_story_overview']
  const ApGaleVeil52 = params74['plot.expected_ending']
  const ApAmberLantern25 = params74['plot.core_conflict']
...
export function ApScarletEmber13(
  ...
  const ApDuskyEmber0 = result.plot_outline
  if (ApDuskyEmber0 && typeof ApDuskyEmber0 === 'object') return ApVineEmber0(ApDuskyEmber0, ApIvoryShard57)
  if (outputBindings.length) {
    const ApDuskyShard44 = ApThornHarbor44(result, outputBindings, ApIvoryShard57)
    if (ApDuskyShard44?.stage_plan?.length) return ApDuskyShard44
  }
  const ApDuskyEmber54 = result.ApDuskyEmber54
  ...
  const ApEmberVeil13 = result.accepted_content
