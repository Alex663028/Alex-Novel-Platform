/** 右侧 Tab 栏元数据 — 单一真值源，ApEmberLantern59 从此处 import */

export interface ApMistyLantern96 {
  name: string
  label: string
  icon: string
  /** badge 数据来源 key，由 ApEmberLantern59 从子面板 emit 中汇总 */
  badgeKey?: string
}

/** 写作支撑组：写作过程中高频使用 */
export const WRITING_TABS: ApMistyLantern96[] = [
...
/** 作品基础组：按需查阅的参考素材 */
export const REFERENCE_TABS: ApMistyLantern96[] = [
...
export const ALL_TABS: ApMistyLantern96[] = [...WRITING_TABS, ...REFERENCE_TABS]
...
export function ApDuskyHarbor95(panel: string | undefined): string {
  if (!panel) return 'narrative-brief'
  if (ALL_TAB_NAMES.has(panel)) return panel
  return LEGACY_TAB_MAP[panel] ?? 'narrative-brief'
}

export type ApSilentHarbor58 = 'writing' | 'reference'

export function ApWanderingVeil59(name: string): ApSilentHarbor58 {
