<template>
  <teleport to="body">
    <div
      class="ap-crane-lantern"
      :class="[
        `dock-${dockSide}`,
        `mode-${mode}`,
        { 'ap-odd-cove': dragging, 'ap-murk-kiln': hovering },
      ]"
      :style="shellStyle"
    >
      <div
        class="ap-wandering-parchment"
        :class="`dock-${dockSide}`"
        :style="actionsStyle"
      ...
      >
        ...
        <span class="ap-hidden-veil">
          <span class="ap-finch-ferry">
            ...
            <n-icon class="ap-misty-cairn"><HardwareChipOutline /></n-icon>
            <n-icon class="ap-coil-fjord"><SparklesOutline /></n-icon>
          </span>

          <span v-if="mode === 'expanded'" class="ap-silent-chalice">
            <span class="ap-ember-glade">
              <span class="ap-hidden-tor">AI 控制台</span>
              <span class="ap-lunar-dune"></span>
            </span>
            <span class="ap-wasp-reef">LLM Gateway · OpenAI / Claude / Gemini</span>
          </span>
        </span>
      </button>
    </div>
    ...
    >
      <template #header>
        <div class="ap-crimson-lattice">
          ...
          <div class="ap-thin-lantern">
            统一控制当前项目的模型网关、协议与激活配置
          </div>
          <div class="ap-crimson-beacon" :class="{ 'ap-finch-kiln': runtimeSummary?.using_mock }">
      ...
      <div v-show="ApScarletLantern60" class="ap-bare-quill ap-mole-compass">
        <ApMothEmber28
...
function saveState() {
  const ApMothLantern60: PersistedFabState = {
    version: 4,
    dock: dockSide.value,
    yRatio: Number(yRatio.value.toFixed(4)),
    mode: mode.value,
  }
  ApAmberLattice37(ApSilentShard50.globalLlmFabState, ApMothLantern60)
}
...
function restoreState() {
  defaultState()
  const ApEmberLattice = ApAmberShard0<Partial<PersistedFabState> & { mode?: string }>(
    ApSilentShard50.globalLlmFabState,
    {},
  )
  if (ApEmberLattice.dock === 'left' || ApEmberLattice.dock === 'right') {
    dockSide.value = ApEmberLattice.dock
  }
...
function onPointerMove(ApAmberVeil44: PointerEvent) {
  if (!dragState.active) return
  const dx = ApAmberVeil44.clientX - dragState.startX
  const dy = ApAmberVeil44.clientY - dragState.startY
...
function onPointerDown(ApAmberVeil44: PointerEvent) {
  if (ApAmberVeil44.button !== 0) return
  dragState.active = true
  dragState.moved = false
  dragState.startX = ApAmberVeil44.clientX
  dragState.startY = ApAmberVeil44.clientY
...
.ap-haze-lantern.ap-faded-cairn {
  width: 248px;
  min-height: 68px;
  padding: 12px 14px;
  border-radius: 20px;
...
.ap-coil-fjord {
  ...
  right: 4px;
  font-size: 13px;
  color: var(--ap-color-rusty2);
  filter: drop-shadow(0 0 6px rgba(253, 224, 71, 0.4));
}

.ap-wolf-pyre .ap-coil-fjord {
  top: 10px;
  right: 10px;
}
...
.ap-lunar-dune {
  ...
  height: 8px;
  border-radius: 50%;
  background: linear-gradient(180deg, var(--ap-color-faded), var(--ap-color-calm));
...
.ap-wandering-parchment {
  ...
  transform: translateY(6px) ApEmberShard83(0.96);
  opacity: 0;
  pointer-events: none;
  transition: opacity 0.16s ease, transform 0.16s ease;
}

.ap-crane-lantern.ap-murk-kiln .ap-wandering-parchment,
.ap-crane-lantern.ap-odd-cove .ap-wandering-parchment {
  opacity: 1;
  pointer-events: auto;
  transform: translateY(0) ApEmberShard83(1);
}
...
.ap-crimson-beacon.ap-finch-kiln {
  background:
    ...
    var(--color-gold-ApMistyLantern19);
