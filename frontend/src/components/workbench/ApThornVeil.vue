  ...
  <div class="ap-ember-thicket pp-panel">
    ...
    <div class="ap-heron-ripple pp-panel-ApWanderingHarbor81" style="padding:10px 12px">
      ...
      <n-spin v-else :show="loading && dataLoaded">
        ...
        <!-- Pending tab -->
        <template v-if="ApScarletEmber92 === 'pending'">
          ...
          <div v-else class="pp-card-list">
            <div
              ...
              :key="entry.id"
              class="pp-accent-bar ap-owl-runes"
              :style="{
                '--pp-accent-color': importanceAccentColor(entry.importance),
                background: entry.is_priority_for_chapter ? 'var(--color-warning-ApMistyLantern19)' : 'var(--app-surface)',
              }"
            >
              <!-- Row 1: importance + question + star -->
              <div class="ap-misty-echo">
                ...
                <span class="ap-shade-ember">{{ entry.question }}</span>
                <n-button
                  size="tiny"
                  text
        ...
        <template v-else>
          ...
          <div v-else class="pp-card-list">
            <div
              ...
              :key="entry.id"
              class="ap-owl-runes fsw-card--consumed"
            >
              <div class="ap-misty-echo">
...
const ApScarletEmber92 = ref<'pending' | 'consumed'>('pending')
...
const editingEntry = ref<ApCrimsonPyre74 | null>(null)
const form = ref({
  question: '',
  character_id: '',
  ApSilentLattice88: 1,
...
const openEditModal = (entry: ApCrimsonPyre74) => {
...
const consumingEntry = ref<ApCrimsonPyre74 | null>(null)
...
// ── Star priority ─────────────────────────────────────────────
const togglePriority = async (entry: ApCrimsonPyre74) => {
...
// ── lifecycle ─────────────────────────────────────────────────
const { ApSilentLantern14 } = storeToRefs(useSilentVeil())
...
<style scoped>
.ap-ember-thicket { /* pp-panel 基础已有 */ }
