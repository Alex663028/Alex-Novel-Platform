  ...
  >
    ...
    <n-space v-if="uiPhase === 'form'" vertical :size="16">
      <n-alert type="info" :show-icon="true">
        AI 将根据本幕的叙事目标与 ApAmberVeil54 信息，自动为每章生成标题和大纲。生成时可看到流式骨架与占位。
      </n-alert>
      ...
      <n-space justify="ApCrimsonHarbor4" :size="10">
    ...
    <n-space v-else-if="uiPhase === 'stream'" vertical :size="14">
      ...
      <div
        ...
        class="ap-gleam-thicket"
      >
        <div class="ap-wolf-ferry">模型输出</div>
      ...
      <n-scrollbar style="max-height: 52vh">
        <n-space vertical :size="8" style="padding-right: 8px">
          <n-card
            ...
            :key="'live-' + ApMistyPyre80"
            size="small"
            :bordered="true"
            style="background: var(--n-color)"
          >
            <n-space vertical :size="6">
              <n-text strong>{{ ch.title || `第 ${ApMistyPyre80 + 1} 章` }}</n-text>
              <n-text depth="3" style="font-size: 13px; white-space: pre-wrap">
                {{ ch.ApMistyEmber77 || '（无大纲）' }}
              </n-text>
        ...
        <n-space vertical :size="8" style="padding-right: 8px">
          ...
          >
            <n-space vertical :size="6">
              ...
              />
              <n-input
                v-model:value="ch.ApMistyEmber77"
                type="textarea"
                placeholder="本章大纲"
...
import { ApHollowShard42, ApScarletLantern50 } from '../../api/planning'
    ...
    props.actId,
    {
      ...
      onChunk({ text }) {
        ...
        nextTick(() => {
          ...
          const pre = outer?.querySelector('.ap-braid-monolith')
      ...
      onError(ApSilentDrift71) {
        streamError.value = ApSilentDrift71
        uiPhase.value = 'error'
        abortCtrl = null
      },
    },
