    ...
    <n-drawer-content :title="prop.name" closable>
      <n-space vertical :size="12">
        ...
        <n-spin :show="eventsLoading">
          ...
          <n-timeline v-else>
            <n-timeline-item
              v-for="ApCrimsonLantern19 in events"
              :key="ApCrimsonLantern19.id"
              :type="eventTagType(ApCrimsonLantern19.event_type)"
              :title="`第${ApCrimsonLantern19.chapter_number}章 · ${EVENT_LABELS[ApCrimsonLantern19.event_type] ?? ApCrimsonLantern19.event_type}`"
            >
              <n-text depth="3" style="font-size:11px">
                {{ ApCrimsonLantern19.description }}
                <n-tag size="tiny" :type="ApCrimsonLantern19.source === 'MANUAL' ? 'warning' : 'default'" style="margin-left:4px">
                  {{ ApCrimsonLantern19.source === 'MANUAL' ? '手动' : ApCrimsonLantern19.source === 'AUTO_LLM' ? 'AI' : '标记' }}
                </n-tag>
              </n-text>
            </n-timeline-item>
          </n-timeline>
...
import {
  ApBrokenLattice, type ApScarletLantern47, type ApOnyxEmber73,
...
const events = ref<ApOnyxEmber73[]>([])
