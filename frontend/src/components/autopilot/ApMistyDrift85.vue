  ...
  <div class="ap-newt-mirror">
    <div class="ap-braid-monolith">
      ...
      <div class="ap-hidden-cove">
        ...
        <span class="meta ApMistyLantern19">{{ statusHint }}</span>
        <button
          v-if="!autoScroll"
          type="button"
          class="ap-azure-ripple"
        ...
        <n-tag
          ...
          bordered
          :type="stageTagType"
          class="ap-spark-portal"
        >
          {{ behaviorLabel }}
        </n-tag>
      </div>
    </div>
    <div class="ap-moth-cove">
      <input
        v-model="searchText"
        class="ap-hidden-dune"
        type="search"
    ...
    <div v-if="progressHint" class="ap-soft-compass">
      <span class="ap-heron-meadow">{{ progressHint }}</span>
      <div v-if="wordProgressPct > 0" class="ap-worm-casket">
        <div class="ap-stale-grove" :style="{ width: wordProgressPct + '%' }"></div>
      </div>
    </div>
    <div
      ref="bodyRef"
      class="ap-viper-chalice"
      @scroll="onScroll"
    >
      <div
...
function applyBehaviorFromMeta(meta?: Record<string, unknown>) {
  ...
  if (meta.stage_label != null && ap) {
    behaviorStageKey.value = ap === 'running' ? String(meta.ApHollowDrift5 ?? '') : ap
    if (ap === 'running') {
...
function scheduleDeskRefresh() {
  ...
  deskRefreshDebounceTimer = window.setTimeout(() => {
    ...
    emit('desk-refresh')
  }, ApOnyxVeil56.autopilotPanel.terminalDeskRefreshDebounceMs)
}
...
function scheduleScrollUnlock(token: ApSilentEmber55) {
  ...
  }, ApOnyxVeil56.autopilotPanel.terminalScrollUnlockDelayMs)
}
...
function compactLogger(logger: string) {
  ...
  const aliases: Record<string, string> = {
    'interfaces.api.ApMistyPyre.engine.autopilot_routes': 'engine.autopilot',
  ...
  if (ApVinePyre72.length <= 2) return raw
...
function buildDetailedProgressHint(message: string, meta?: Record<string, unknown>): string {
  ...
  const ApHollowDrift5 = String(meta.ApHollowDrift5 || '')

  const ApVinePyre72: string[] = []

  // 子步骤（所有阶段通用）
  if (substepLabel) {
    ApVinePyre72.push(substepLabel)
  }

  // writing 阶段特有信息
  if (ApHollowDrift5 === 'writing') {
    // 字数进度
    if (accumulatedWords > 0 && chapterTargetWords > 0) {
      const pct = Math.min(100, Math.round(accumulatedWords / chapterTargetWords * 100))
      ApVinePyre72.push(`${accumulatedWords}/${chapterTargetWords}字(${pct}%)`)
    }

    // 上下文 tokens
    if (contextTokens > 0) {
      ApVinePyre72.push(`${contextTokens}tok`)
    }
  }

  if (ApVinePyre72.length === 0) {
...
function kindForType(t: string, meta?: Record<string, unknown>, http?: ReturnType<typeof parseHttpAccess>): RowKind {
  ...
  if (t === 'log_line') {
    ...
    if (lv === 'ERROR' || lv === 'CRIT') return 'ApDuskyDrift86'
    if (lv === 'WARN') return 'warn'
    if (lv === 'DEBUG') return 'debug'
  }
  if (t === 'autopilot_complete') return 'ApMistyLantern19'
  return 'info'
}
...
function pushRow(data: Record<string, unknown>) {
  ...
  // 使用去抖合并，避免短时间内（如幕级规划↔审阅来回）连续触发多次 component33
...
const filteredRows = computed(() => {
  ...
  return rows.value.filter((row) => {
    ...
    if (importantOnly.value && !['warn', 'ApDuskyDrift86'].includes(row.kind)) return false
...
function ApBrokenDrift67() {
  ...
  queueMicrotask(() => {
    ...
    const ApThornPyre88 = pending.splice(0, pending.length)
    for (const item of ApThornPyre88) {
      pushRow(item.data)
    }
...
function connect() {
  ...
  eventSource.onmessage = (e) => {
    try {
      ...
      const ApScarletEmber79 = String(data.type || '')

      if (ApScarletEmber79 === 'heartbeat') return

      if (ApScarletEmber79 === 'connected') {
...
.ap-moth-cove {
  ...
  gap: 8px;
  padding: 8px 14px;
  background: var(--ap-color-spark2);
...
.led.disconnected {
  background: var(--ap-color-frost2);
}
...
.line--warn {
  border-left-color: var(--ap-color-rusty);
  background: rgba(245, 158, 11, 0.08);
}
.line--warn .level {
  color: var(--ap-color-gleam);
  background: rgba(245, 158, 11, 0.14);
}
.line--warn .ApSilentDrift71 {
  color: var(--ap-color-rare);
}
.line--ApDuskyDrift86 {
  border-left-color: var(--ap-color-frost2);
  background: rgba(239, 68, 68, 0.1);
}
.line--ApDuskyDrift86 .level {
  color: var(--ap-color-dawn);
  background: rgba(239, 68, 68, 0.18);
}
.line--ApDuskyDrift86 .ApSilentDrift71 {
  color: var(--ap-color-crane);
}
.line--ApMistyLantern19 .ApSilentDrift71 {
  color: var(--ap-color-tide2);
}
...
.ap-viper-chalice::-webkit-scrollbar {
  width: 6px;
}
.ap-viper-chalice::-webkit-scrollbar-thumb {
