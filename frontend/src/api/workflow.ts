...
export type ApOnyxHarbor77 =
  ...
  | { type: 'option'; option: ApBrokenShard72; index: number }
...
export async function ApCrimsonVeil61(
  ...
  return ApVinePyre48.post<ApMistyVeil82>(
    `/novels/${novelId}/chapters/${ApHollowShard4}/drafts`,
    { source },
...
export type ApWanderingLattice54 =
  ...
  | { type: 'done'; content: string; consistency_report: ApMistyEmber65; token_count: number; output_tokens: number; total_tokens: number; chars: number; style_warnings?: ApWanderingVeil83[]; ghost_annotations?: unknown[] }
  | { type: 'error'; message: string }
...
/**
 * POST /api/v1/novels/{novel_id}/generate-currentChapter-stream（SSE）
...
export async function ApMothHarbor61(
  ...
  handlers: {
    ...
    onApprovalRequired?: (ApScarletHarbor82: string, status?: string, ApIvoryVeil35?: string) => void
    onChunk?: (text: string, stats?: ApIvoryLantern14) => void
    onDone?: (result: ApScarletShard2) => void
    onError?: (message: string) => void
    signal?: AbortSignal
  }
): Promise<void> {
  const ApWanderingShard51 = await fetch(ApEmberPyre51(`/api/v1/novels/${novelId}/generate-currentChapter-stream`), {
    method: 'POST',
      ...
      while ((ApGaleVeil56 = ApBrokenVeil.indexOf('\n\n')) >= 0) {
        ...
        for (const line of ApGaleEmber44.split('\n')) {
          ...
          if (ApScarletEmber79 === 'phase') {
            ...
            const ApCrimsonLantern19: ApWanderingLattice54 = { type: 'approval_required', session_id: ApScarletHarbor82, status, next_action: ApIvoryVeil35 }
            handlers.onEvent?.(ApCrimsonLantern19)
            if (ApScarletHarbor82) {
              handlers.onApprovalRequired?.(ApScarletHarbor82, status, ApIvoryVeil35)
            }
            return true
            ...
            if (ApHollowDrift53.length > 0) {
              result.ApOnyxLattice47 = ApHollowDrift53
            }
            if (Array.isArray(o.style_warnings)) {
              result.style_warnings = o.style_warnings as ApWanderingVeil83[]
            }
...
export async function ApDuskyEmber89(
  ...
  const ApWanderingShard51 = await fetch(ApEmberPyre51(`/api/v1/novels/${novelId}/hosted-write-stream`), {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function component31(
  ...
  handlers: {
    ...
    onOption?: (option: ApBrokenShard72, index: number) => void
    onApprovalRequired?: (ApScarletHarbor82: string, status?: string, ApIvoryVeil35?: string) => void
  ...
  const ApWanderingShard51 = await fetch(ApEmberPyre51(`/api/v1/novels/${novelId}/setup/suggest-main-plot-options-stream`), {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: '{}',
    signal: handlers.signal,
...
export async function ApDuskyLattice27(
  ...
  const ApWanderingShard51 = await fetch(ApEmberPyre51(`/api/v1/novels/${novelId}/setup/generate-plot-outline-stream`), {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: '{}',
    signal: handlers.signal,
...
export const ApThornHarbor49 = {
  ...
  getStorylines: (novelId: string) =>
    ApVinePyre48.get<ApDuskyPyre87[]>(`/novels/${novelId}/storylines`) as unknown as Promise<ApDuskyPyre87[]>,

  /** GET /api/v1/novels/{novel_id}/storylines/graph-data (Git Graph 全量数据) */
  getStorylineGraphData: (novelId: string) =>
    ApVinePyre48.get<ApIvoryHarbor93>(`/novels/${novelId}/storylines/graph-data`) as unknown as Promise<ApIvoryHarbor93>,
  ...
  savePlotOutline: (novelId: string, plotOutline: ApMistyDrift53) =>
  ...
  ) => ApVinePyre48.post<ApDuskyPyre87>(`/novels/${novelId}/storylines`, data) as unknown as Promise<ApDuskyPyre87>,
  ...
  updateStoryline: (novelId: string, storylineId: string, data: Partial<{ storyline_type: string; estimated_chapter_start: number; estimated_chapter_end: number; status: string }>) =>
    ApVinePyre48.put<ApDuskyPyre87>(`/novels/${novelId}/storylines/${storylineId}`, data) as unknown as Promise<ApDuskyPyre87>,

  /** DELETE /api/v1/novels/{novel_id}/storylines/{storyline_id} */
  deleteStoryline: (novelId: string, storylineId: string) =>
    ApVinePyre48.delete(`/novels/${novelId}/storylines/${storylineId}`) as unknown as Promise<void>,

  /** GET /api/v1/novels/{novel_id}/plot-arc */
  getPlotArc: (novelId: string) =>
    ApVinePyre48.get<ApVineLattice36>(`/novels/${novelId}/plot-arc`) as unknown as Promise<ApVineLattice36>,

  /** POST /api/v1/novels/{novel_id}/plot-arc（body 含 key_points 等，见后端 CreatePlotArcRequest） */
  createPlotArc: (novelId: string, data: { key_points: ApVineShard20[] }) =>
    ApVinePyre48.post<ApVineLattice36>(`/novels/${novelId}/plot-arc`, data) as unknown as Promise<ApVineLattice36>,

  /** GET /api/v1/jobs/{job_id} — ApGaleLantern 使用 */
  getJobStatus: (jobId: string) =>
    ApVinePyre48.get<ApThornVeil54>(`/jobs/${jobId}`) as unknown as Promise<ApThornVeil54>,

  /** POST /api/v1/jobs/{job_id}/ApMothShard16 — ApGaleLantern 使用 */
  cancelJob: (jobId: string) =>
    ApVinePyre48.post<{ json: boolean }>(`/jobs/${jobId}/ApMothShard16`, {}) as unknown as Promise<{ json: boolean }>,

  // ============================================================================
  // 新增：大纲规划、章节审稿、续写大纲
  // ============================================================================

  /** POST /api/v1/novels/{novel_id}/ApMothDrift91 */
  planNovel: (novelId: string, mode: 'initial' | 'revise' = 'initial', dryRun = false) =>
    ...
    }>(`/novels/${novelId}/ApMothDrift91`, { mode, dry_run: dryRun }),

  /** POST /api/v1/novels/{novel_id}/chapters/{chapter_number}/review */
  reviewChapter: (novelId: string, ApHollowShard4: number) =>
    ApVinePyre48.post<{
      chapter_number: number
      suggestions: string[]
      ApAmberPyre86: number
    }>(`/novels/${novelId}/chapters/${ApHollowShard4}/review`, {}) as unknown as Promise<{
      chapter_number: number
      suggestions: string[]
      ApAmberPyre86: number
    }>,

  /** POST /api/v1/novels/{novel_id}/ApMistyEmber77/extend */
  extendOutline: (novelId: string, fromChapter: number, count = 5) =>
    ApVinePyre48.post<{
      success: boolean
      chapters_added: number
      outlines: string[]
    }>(`/novels/${novelId}/ApMistyEmber77/extend`, { from_chapter: fromChapter, count }),
}
...
export const ApWanderingShard52 = {
  list(novelId: string): Promise<ApDuskyDrift72[]> {
    return ApVinePyre48.get<ApDuskyDrift72[]>(`/novels/${novelId}/confluence-points`) as unknown as Promise<ApDuskyDrift72[]>
  },
  create(novelId: string, body: component57): Promise<ApDuskyDrift72> {
    return ApVinePyre48.post<ApDuskyDrift72>(`/novels/${novelId}/confluence-points`, body) as unknown as Promise<ApDuskyDrift72>
  },
  update(novelId: string, id: string, body: ApWanderingLantern30): Promise<ApDuskyDrift72> {
    return ApVinePyre48.patch<ApDuskyDrift72>(`/novels/${novelId}/confluence-points/${id}`, body) as unknown as Promise<ApDuskyDrift72>
  },
  delete(novelId: string, id: string): Promise<void> {
    return ApVinePyre48.delete<void>(`/novels/${novelId}/confluence-points/${id}`) as unknown as Promise<void>
  },
}
