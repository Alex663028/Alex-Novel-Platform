...
# ---------------------------------------------------------------------------
# Shared httpx client singletons
# ---------------------------------------------------------------------------
# A single connection pool per process for sync (Anthropic SDK) and a single
# one for async (AsyncAnthropic SDK + streaming).  The getters recreate the
# client if a previous caller closed it so the pool always stays usable.
# ---------------------------------------------------------------------------
_SHARED_USER_AGENT = "Alex/1.0"

_shared_sync_http_client: httpx.Client | None = None
_shared_async_http_client: httpx.AsyncClient | None = None


def _get_shared_sync_client(timeout: httpx.Timeout) -> httpx.Client:
    """Return a shared sync ``httpx.Client`` (lazy singleton)."""
    global _shared_sync_http_client
    if _shared_sync_http_client is None or _shared_sync_http_client.is_closed:
        _shared_sync_http_client = httpx.Client(
            timeout=timeout,
            trust_env=False,
            headers={"user-agent": _SHARED_USER_AGENT},
        )
    return _shared_sync_http_client


def _get_shared_async_client(timeout: httpx.Timeout) -> httpx.AsyncClient:
    """Return a shared async ``httpx.AsyncClient`` (lazy singleton).

    Used by both the AsyncAnthropic SDK *and* the streaming endpoint so they
    share one connection pool.
    """
    global _shared_async_http_client
    if _shared_async_http_client is None or _shared_async_http_client.is_closed:
...
class AnthropicProvider(BaseProvider):
    ...
    def __init__(self, settings: Settings):
        ...
        # 单例共享 httpx client：避免重复的连接池，跨 Provider 实例复用连接。
        _sdk_timeout = build_httpx_timeout(settings.http_timeout_settings)
        self._http_client_sync = _get_shared_sync_client(_sdk_timeout)
        self._http_client_async = _get_shared_async_client(_sdk_timeout)
    ...
    async def _stream_via_httpx(
        ...
        async with self._http_client_async.stream(
            "POST",
            url,
            headers=headers,
