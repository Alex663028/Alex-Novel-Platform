...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(description='AlexCLI')
    ...
    if parsed_args.command == 'serve':
        ...
        if _port_in_use(_port):
            ...
            try:
                    ...
                    for line in r.stdout.splitlines():
                        ...
                        if len(parts) >= 5 and f":{_port}" in parts[1] and parts[3] == "LISTENING":
                            try:
                                ...
                                pass
                    if _pid:
                        print(f"[Alex] 端口 {_port} 被占用 (PID={_pid})，正在释放...")
                ...
                print(f"[Alex] 释放端口失败: {_e}")
            # 等待端口释放
            for _ in range(15):
                if not _port_in_use(_port):
                    print(f"[Alex] 端口 {_port} 已释放")
                    break
                _t.sleep(0.3)
            else:
                print(f"[Alex] 警告：端口 {_port} 仍被占用，启动可能失败")
